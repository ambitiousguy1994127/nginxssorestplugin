...
ngx_int_t get_ngx_http_request_url(ngx_http_request_t *r, ngx_http_variable_value_t *v, int useServerNameAsDefault)
{
    logDebug(r->connection->log, 0, "Starting URL parsing");
    char *scheme = NULL;
    char *hostname = NULL;
    char *url = NULL;
    u_char *last = NULL;
    ...
    if (v == NULL || v->not_found) {
        logError(r->connection->log, 0, "Could not determine URL scheme");
        return NGX_OK;
    }
    len = v->len + 3;
    ...
    if (v == NULL || v->not_found) {
        if (useServerNameAsDefault)
        {
            get_ngx_http_request_server_name(r, v);
            if (v == NULL || v->not_found) {
                logWarn(r->connection->log, 0, "Could not determine ServerName");
                ngx_free(scheme);
                return NGX_OK;
            }
            *(v->data+v->len) = '\0';
            logDebug(r->connection->log, 0, "No host header in request, using default server name: %s", v->data);
        }
        else {
            logWarn(r->connection->log, 0, "Could not determine URL hostname");
            ngx_free(scheme);
            return NGX_OK;
        }
    }
    ...
    int len2 = len + v->len;
    hostname = (char *) ngx_calloc(len2, r->connection->log);
    last = ngx_copy(hostname, scheme, len);
    last = ngx_copy(last, v->data, v->len);

    get_ngx_http_request_uri(r, v);
    if (v == NULL || v->not_found) {
        logError(r->connection->log, 0, "Could not determine URL URI");
        ngx_free(scheme);
        ngx_free(hostname);
        return NGX_OK;
    }
    int len3 = len2 + v->len;
    url = ngx_pnalloc(r->pool, len3 + 1);
    last = ngx_copy(url, hostname, len2);
    last = ngx_copy(last, v->data, v->len);
    *last = '\0';
    v->data = (u_char*) url;
    v->len = len3;

    logDebug(r->connection->log, 0, "Parsed URL: %s", url);
...
{
    return get_ngx_http_request_headers(r, v, offsetof(ngx_http_request_t, headers_in.content_type));
}
...
{
    return get_ngx_http_request_variable(r, v, offsetof(ngx_http_request_t, unparsed_uri));
}

ngx_int_t get_ngx_http_request_hostname(ngx_http_request_t *r, ngx_http_variable_value_t *v)
{
    return get_ngx_http_request_headers(r, v, offsetof(ngx_http_request_t, headers_in.host));
}
...
{
#if (NGX_HTTP_HEADERS) 
    return get_ngx_http_request_headers(r, v, offsetof(ngx_http_request_t, headers_in.accept_language));
#else
    v->not_found = 1;
    return NGX_OK;
#endif
...
{
#if (NGX_HTTP_HEADERS) 
    return get_ngx_http_request_headers(r, v, offsetof(ngx_http_request_t, headers_in.accept));
#else
    v->not_found = 1;
    return NGX_OK;
#endif
}
// ngx_int_t get_ngx_http_request_host(ngx_http_request_t *r, ngx_http_variable_value_t *v)
// {
...
{
#if (NGX_HTTP_GZIP)
    return get_ngx_http_request_headers(r, v, offsetof(ngx_http_request_t, headers_in.accept_encoding));
#else
    v->not_found = 1;
    return NGX_OK;
#endif
...
{
    return get_ngx_http_request_headers(r, v, offsetof(ngx_http_request_t, headers_in.user_agent));
}

ngx_int_t get_ngx_http_request_extension(ngx_http_request_t *r, ngx_http_variable_value_t *v)
{
    return get_ngx_http_request_variable(r, v, offsetof(ngx_http_request_t, exten));
}

ngx_int_t get_ngx_http_request_args(ngx_http_request_t *r, ngx_http_variable_value_t *v)
{
    return get_ngx_http_request_variable(r, v, offsetof(ngx_http_request_t, args));
}

ngx_int_t get_ngx_http_request_document_uri(ngx_http_request_t *r, ngx_http_variable_value_t *v)
{
    return get_ngx_http_request_variable(r, v, offsetof(ngx_http_request_t, uri));
}
